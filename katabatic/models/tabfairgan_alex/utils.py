...
def get_ohe_data_fair(
    ...
    ohe, scaler, discrete_columns_ordereddict, continuous_columns_list, numerical_array, ohe_array = get_cont_cat_transform(
        df)

    S_start_index = len(continuous_columns_list) + sum(
        list(
            discrete_columns_ordereddict.values())[
            :list(
                discrete_columns_ordereddict.keys()).index(S)])
    Y_start_index = len(continuous_columns_list) + sum(
        list(
            discrete_columns_ordereddict.values())[
            :list(
                discrete_columns_ordereddict.keys()).index(Y)])

    if ohe.categories_[
            list(discrete_columns_ordereddict.keys()).index(S)][0] == S_under:
        underpriv_index = 0
        priv_index = 1
    else:
        underpriv_index = 1
        priv_index = 0
    if ohe.categories_[
            list(discrete_columns_ordereddict.keys()).index(Y)][0] == Y_desire:
        desire_index = 0
        undesire_index = 1
    else:
        desire_index = 1
...
def get_original_data(
    ...
    df_cat = pd.DataFrame(
        df_ohe_cats,
        columns=df_orig.select_dtypes('object').columns)
...
def prepare_data_fair(
    ...
    ohe, scaler, discrete_columns, continuous_columns, df_transformed, S_start_index, Y_start_index, underpriv_index, priv_index, undesire_index, desire_index = get_ohe_data_fair(
...
def prepare_data_nofair(df: pd.DataFrame, batch_size: int) -> Tuple[
    ...
    ohe, scaler, discrete_columns, continuous_columns, df_transformed = get_ohe_data_nofair(
        df)
