...
class TFG:
    ...
    def __init__(
        ...
        self.fair_epochs = fairness_config.get(
            'fair_epochs', 0) if fairness_config else 0
        ...
        if fairness_config and self.fair_epochs > 0:
            ...
            if not isinstance(self.lamda, float):
                raise TypeError(
                    "When fair_epochs is > 0, 'lamda' must be a float.")
            if not isinstance(self.S, str):
                raise TypeError(
                    "When fair_epochs is > 0, 'S' must be a string.")
            if not isinstance(self.Y, str):
                raise TypeError(
                    "When fair_epochs is > 0, 'Y' must be a string.")
            if not isinstance(self.S_under, str):
                raise TypeError(
                    "When fair_epochs is > 0, 'S_under' must be a string.")
            if not isinstance(self.Y_desire, str):
                raise TypeError(
                    "When fair_epochs is > 0, 'Y_desire' must be a string.")
        else:
            self.lamda = None
            self.S = None
            self.Y = None
    ...
    def prepare_data(self) -> None:
        if self.fair_epochs > 0:
            self.ohe, self.scaler, self.input_dim, self.discrete_columns, self.continuous_columns, self.train_dl, self.data_train, self.data_test, self.S_start_index, self.Y_start_index, self.underpriv_index, self.priv_index, self.undesire_index, self.desire_index = prepare_data_fair(
                self.df, self.batch_size, self.S, self.Y, self.S_under, self.Y_desire)

        else:
            self.ohe, self.scaler, self.input_dim, self.discrete_columns, self.continuous_columns, self.train_dl, self.data_train, self.data_test = prepare_data_nofair(
                self.df, self.batch_size)
    ...
    def train(self) -> None:
        ...
        self.generator = Generator(
            ...
            self.discrete_columns).to(
            self.device)

        self.critic = Critic(self.input_dim).to(self.device)

        if self.fair_epochs > 0:
            self.second_critic = FairLossFunc(
                self.S_start_index,
                self.Y_start_index,
                self.underpriv_index,
                self.priv_index,
                self.undesire_index,
                self.desire_index).to(
                self.device)

        self.gen_optimizer = torch.optim.Adam(
            self.generator.parameters(), lr=0.0002, betas=(0.5, 0.999))
        self.gen_optimizer_fair = torch.optim.Adam(
            self.generator.parameters(), lr=0.0001, betas=(0.5, 0.999))
        self.crit_optimizer = torch.optim.Adam(
            self.critic.parameters(), lr=0.0002, betas=(0.5, 0.999))
                ...
                for data in self.train_dl:
                    ...
                    for k in range(crit_repeat):
                        ...
                        gradient = get_gradient(
                            self.critic, data[0], fake.detach(), epsilon)
                    ...
                    if i + 1 > (self.epochs - self.fair_epochs):
                        ...
                        gen_fair_loss = self.second_critic(
                            fake_2, crit_fake_pred, self.lamda)
    ...
    def generate_fake_df(self, num_rows: int) -> pd.DataFrame:
        with torch.no_grad():
            ...
            fake_df = get_original_data(
                fake_numpy_array, self.df, self.ohe, self.scaler)
