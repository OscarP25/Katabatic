...
class CombU(nn.Module):
    def __init__(self, size: int, negative_slope: float = 0.2):
        ...
        self.relu_dims = torch.from_numpy(relu_dims)
        self.elu_dims = torch.from_numpy(elu_dims)
        self.log_dims = torch.from_numpy(log_dims)

        self.relu = nn.ReLU(inplace=False)
        self.elu = nn.ELU(inplace=False)
        self.lrelu = nn.LeakyReLU(negative_slope, inplace=False)

        self.register_buffer("relu_dims_buf", self.relu_dims)
        self.register_buffer("elu_dims_buf", self.elu_dims)
        self.register_buffer("log_dims_buf", self.log_dims)
...
class EncoderOutput:
    def __init__(self, out: torch.Tensor, info: Optional[torch.Tensor] = None):
        self.out = out
        self.info = info
    ...
    def __init__(
        ...
        for i, o in zip(in_dims[:-1], out_dims[:-1]):
            layers.append(
                nn.Sequential(
                    nn.Linear(i, o),
                    CombU(o, negative_slope=0.2),
                    nn.LayerNorm(o),
                    nn.Dropout(0.2),
                )
            )

        act_layer = nn.Identity() if act else nn.ReLU(True)
    ...
    def __init__(
        ...
        for i, o in zip(in_dims[:-1], out_dims[:-1]):
            layers.append(
                nn.Sequential(
                    nn.Linear(i, o),
                    CombU(o, negative_slope=0.0),
                    nn.LayerNorm(o),
                    nn.Dropout(0.2),
                )
            )

        layers.append(nn.Linear(in_dims[-1], out_dims[-1]))
        self.layers = nn.Sequential(*layers)
...
class AEOutput:
    def __init__(self, out: torch.Tensor, mask: Optional[torch.Tensor] = None):
        self.out = out
        self.mask = mask
...
class AutoEncoder(nn.Module):
    def __init__(
        ...
        self.decoder = Decoder(
            in_dim=embed_dim + cont_noise_dim + cat_noise_dim,
            out_dim=data_dim,
            hidden_dim=hidden_dim,
            n_layers=n_layers,
        )
    ...
    def forward(
        self,
        known: torch.FloatTensor,
        known_indicator: torch.BoolTensor,
        ...
        data = torch.cat([known, known_indicator.float()], dim=-1)
        encoded = self.encoder(data).out

        cont_noise = torch.randn(
            known.shape[0], self._cont_noise_dim, device=known.device
        )
        cat_noise = torch.randint(
            0, 2, size=(known.shape[0], self._cat_noise_dim), device=known.device
        )
...
class TableReconLoss(nn.Module):
    ...
    def forward(
        ...
        unknown_weights = (
            (known_indicator.sum(dim=-1) / known_indicator.shape[1])
            * (self._max_weight - self._min_weight)
            + self._min_weight
        )

        loss_weights = (
            known_indicator.float()
        ...
        for i, (w, t) in enumerate(self.span_info):
            if t == "onehot":
                col_loss = self.ce(
                    data[:, st : st + w], target[:, st : st + w].argmax(dim=-1)
                )
            else:
                col_loss = self.smooth_l1(
                    torch.tanh(data[:, st : st + w]), target[:, st : st + w]
                )[:, 0]

            losses[:, i] = col_loss
            st += w

        return ReconLossOutput(losses, loss_weights)
...
class InteractionFactor(nn.Module):
    def __init__(self, span_info: SPANS):
        ...
        self.span_info = span_info
        st = 0
        self.all_st, self.all_ed = [], []

        for w, _ in span_info:
    ...
    def forward(self, data: torch.FloatTensor):
        ...
        for i in range(len(self.span_info) - 1):
            ...
            for j in range(i + 1, len(self.span_info)):
                ...
                prod = (col * ocol.unsqueeze(1)).view(data.shape[0], -1)
                all_mean.append(prod.mean(dim=0))
                all_std.append(prod.std(dim=0))
...
class GumbelSoftmax(nn.Module):
    def forward(
        self, x: torch.FloatTensor, temperature: Union[float, torch.FloatTensor] = 1
    ) -> torch.FloatTensor:
...
class TableActivation(nn.Module):
    ...
    def forward(
        self, data: torch.FloatTensor, temperature: Union[float, torch.FloatTensor] = 1
    ) -> torch.FloatTensor:
        st = 0
        ...
        for w, t in self.span_info:
            if t == "onehot":
                new_data[:, st : st + w] = self.softmax(
                    data[:, st : st + w], temperature
                )
            else:
...
class GradientPenalty(nn.Module):
    ...
    def forward(self, real: torch.FloatTensor, fake: torch.FloatTensor):
        if self.lambda_ == 0:
        ...
        penalty = ((gradients.norm(2, dim=1) - 1) ** 2).mean() * self.lambda_
        return penalty

    def _slerp(
        self, val: torch.FloatTensor, low: torch.FloatTensor, high: torch.FloatTensor
    ):
        ...
        omega = torch.acos((low_norm * high_norm).sum(1).clamp(-1, 1)).view(-1, 1)
