...
class CopulaGANModel(BaseModel):
    """
    CopulaGAN for tabular data generation.
    
    Combines copula-based preprocessing with GAN training:
    1. Copula Transform: Models marginal distributions → transforms to standard normal
    2. GAN Training: CTGAN-style generator/discriminator on transformed data
    3. Inverse Transform: Converts generated samples back to original space
    
    Key advantages:
    - Better handles non-Gaussian distributions
    - Preserves correlation structure
    - Improves statistical resemblance to real data
    """
    ...
    def train(
        ...
        **kwargs,
        ...
        # Schema & copula transformers
        self.schema = infer_schema(df)
        fit_copula_transformers(df, self.schema)
        ...
        if backend == "torch":
            ...
            if torch is None or nn is None or data_utils is None or F is None:
                ...
                # Copula transform
                enc, self.cat_blocks, self.output_order = copula_transform(df, self.schema)
                ...
                print(f"[CopulaGAN] Training with copula preprocessing")
        ...
        print(f"[CopulaGAN] Synthetic data saved:\n  X -> {x_path_out}\n  y -> {y_path_out}")
        return self
            ...
            if torch is None:
                ...
                with torch.no_grad():
                    ...
                    for _ in range(steps):
                        ...
                        df_batch = copula_inverse_transform(enc_fake, self.schema)
