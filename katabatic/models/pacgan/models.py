...
class PacGANModel(BaseModel):
    """
    PacGAN (Packing GAN) for tabular data generation.
    
    PacGAN addresses mode collapse by modifying the discriminator to evaluate
    multiple (packed) samples simultaneously instead of single samples. This
    makes it harder for the generator to fool the discriminator with repetitive
    outputs, thus mitigating mode collapse.
    
    Key difference from standard GAN:
    - Discriminator receives 'm' packed samples (default m=3)
    - Samples are concatenated: (batch_size//m, m*input_dim)
    - This allows the discriminator to detect lack of diversity
    """
    ...
    def _build_torch_networks(self, torch, nn) -> None:
        """Build generator and packing discriminator networks."""
        in_g = self.cfg["noise_dim"] + self._cond_dim
        out_g = self._enc_dim
        
        # Discriminator input is packed: pack_size * (enc_dim + cond_dim)
        pack_size = self.cfg["pack_size"]
        in_d = pack_size * (self._enc_dim + self._cond_dim)
    ...
    def _forward_generator(self, torch, z, cond):
        """Generate samples from noise and conditional vectors."""
        logits = self.generator(torch.cat([z, cond], dim=1))
        return self._gumbelize_cats(logits)

    def _pack_samples(self, torch, x_enc, cond):
        """
        Pack samples for the discriminator.
        
        Packs m samples together by concatenating them along the feature dimension.
        Input: (batch_size, enc_dim), (batch_size, cond_dim)
        Output: (batch_size//pack_size, pack_size * (enc_dim + cond_dim))
        """
        pack_size = self.cfg["pack_size"]
        batch_size = x_enc.size(0)
        
        # Ensure batch_size is divisible by pack_size
        if batch_size % pack_size != 0:
            # Truncate to nearest multiple
            new_batch = (batch_size // pack_size) * pack_size
            x_enc = x_enc[:new_batch]
            cond = cond[:new_batch]
            batch_size = new_batch
        
        # Concatenate x_enc and cond
        combined = torch.cat([x_enc, cond], dim=1)  # (batch_size, enc_dim + cond_dim)
        
        # Reshape to pack samples
        packed_batch_size = batch_size // pack_size
        feature_dim = combined.size(1)
        
        # Reshape: (batch_size, feature_dim) -> (packed_batch_size, pack_size, feature_dim)
        packed = combined.view(packed_batch_size, pack_size, feature_dim)
        
        # Flatten the packed dimension: (packed_batch_size, pack_size * feature_dim)
        packed = packed.view(packed_batch_size, -1)
        
        return packed

    def _forward_discriminator(self, torch, x_enc, cond):
        """Forward pass through packing discriminator."""
        packed = self._pack_samples(torch, x_enc, cond)
        return self.discriminator(packed)
        ...
        **kwargs,
        ...
        if backend == "torch":
            ...
            if torch is None or nn is None or data_utils is None or F is None:
                ...
                # DataLoader - ensure batch_size is divisible by pack_size
                pack_size = self.cfg["pack_size"]
                batch_size = self.cfg["batch_size"]
                # Adjust batch_size to be divisible by pack_size
                batch_size = (batch_size // pack_size) * pack_size
                if batch_size == 0:
                ...
                loader = DataLoader(TensorDataset(
                    enc_t, cond_t), batch_size=batch_size, shuffle=True, drop_last=True)
                ...
                def _wgangp_gradient_penalty(discriminator, real_packed, fake_packed, device, lambda_gp: float = 10.0):
                    """Gradient penalty for WGAN-GP with packed samples."""
                    batch_size = real_packed.size(0)
                    alpha = torch.rand(batch_size, 1, device=device)
                    alpha = alpha.expand_as(real_packed)
                    interpolates = alpha * real_packed + (1 - alpha) * fake_packed
                ...
                print(f"[PacGAN] Training with pack_size={pack_size}, batch_size={batch_size}")
                ...
                for epoch in range(self.cfg["epochs"]):
                    ...
                    for real_batch, real_cond in loader:
                        ...
                        for _ in range(self.cfg["n_critic"]):
                            ...
                            # Pack samples before discriminator
                            real_packed = self._pack_samples(torch, real_batch, real_cond)
                            fake_packed = self._pack_samples(torch, fake_batch, cond_fake)
                            
                            d_real = D(real_packed).mean()
                            d_fake =D(fake_packed).mean()
                            
                            if self.cfg["use_gradient_penalty"]:
                                gp = _wgangp_gradient_penalty(
                                    D, real_packed, fake_packed, self._device, self.cfg["lambda_gp"])
                    ...
                    if (epoch + 1) % 10 == 0:
                        ...
                        print(f"Epoch {epoch+1}/{self.cfg['epochs']}, D_loss: {avg_d_loss:.4f}, G_loss: {avg_g_loss:.4f}")
        ...
        if not synth_dir:
            ...
            synth_dir = os.path.join("synthetic", dataset_name, "pacgan")
