...
class CODI(Model):
    ...
    def __init__(
        ...
        lr_dis: float = 2e-3,
        grad_clip: float = 1.0,
        # Contrastive weights (not used explicitly in this simplified version)
    ...
    def train(self, dataset_dir: str, synthetic_dir: str, **kwargs) -> "CODI":
        ...
        # One-hot dimension for categorical
        if self.has_categorical_:
            onehot_dim_cat = int(np.sum(self.num_classes_))
        else:
            onehot_dim_cat = 1

        # Build models (cond_dim of continuous model = one-hot dim)
        ...
        # ------------------------------------------------------------------
        # Generate synthetic data (encoded), then reorder columns to match
        # original training column order to keep sklearn happy in TSTR.
        # ------------------------------------------------------------------
        logger.info(f"\nGenerating {len(df_train)} synthetic samples...")
        df_synth = self.sample(len(df_train), return_encoded=True)

        # 🔹 Reorder synthetic columns to match original training order
        if "column_order" in self.schema_:
            col_order = self.schema_["column_order"]
            col_order_present = [c for c in col_order if c in df_synth.columns]
            df_synth = df_synth[col_order_present]
        ...
        for col in cat_cols:
            if col in df_synth.columns:
                ...
                df_synth[col] = (
                    df_synth[col].clip(0, max_val).round().astype(int)
                )
        ...
        # Save X/y synthetic separately if target exists
        if y_train is not None:
            y_name = y_train.name if hasattr(y_train, "name") else "target"
    ...
    def _build_models(self, dim_con: int, dim_cat: int):
        ...
        # Continuous model (conditioned on one-hot categorical)
        ...
        # Discrete model (conditioned on continuous)
    ...
    def _fit(self, data_con: np.ndarray, data_cat: np.ndarray):
        """Fit the model using co-evolving training."""
    ...
    def _train_step(
        ...
        """
        Single training step for continuous + discrete diffusion.

        - Continuous: Gaussian diffusion on x_con, conditioned on one-hot(x_cat)
        - Categorical: Multinomial diffusion on x_cat
        """
        ...
        if self.has_continuous_:
            cond_onehot = self._to_onehot(x_cat).to(self.device)

            loss_con_total = self.trainer_con_(x_con, t, cond_onehot)
    ...
    def sample(
        ...
        Args:
            ...
            conditional: (unused) optional conditioning
            return_encoded: If True, return encoded values

        Returns:
            DataFrame of synthetic samples (encoded or decoded)
        """
    ...
    def _sample_reverse(
        ...
        """Reverse diffusion sampling for both continuous and categorical parts."""
        x_t_con = x_T_con
        x_t_cat = log_x_T_cat
        ...
        for t_step in reversed(range(self.n_steps)):
            ...
            if self.has_continuous_:
                x_t_con = self.sampler_con_.p_sample(x_t_con, t, x_t_cat)
